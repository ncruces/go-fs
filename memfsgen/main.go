...
func init() {
	var fs = {{.Variable}}
	{{- range .Assets}}
	fs.CreateString({{printf "%#v" .Name}}, {{printf "%#v" .Type}}, time.Unix({{.Time}}, 0), {{.Size}}, ""
		{{- range .Lines}}+
		"{{.}}"
    {{- end}})
	{{- end}}
...
func main() {
	...
	pkgName := flag.String("pkg", "", "package name (default: lowercase directory name of <target-file>)")
	tagName := flag.String("tag", "", "build tags")
...
func walk(root string, assets chan<- Asset) {
	filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() {
			...
			assets <- Asset{path, sniff(path, data), modtime.Unix(), info.Size(), lines}
